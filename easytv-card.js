...
const CARD_STYLES = `
  ...
  .compact-single {
    ...
    padding: 10px 14px;
    border-radius: var(--easytv-card-border-radius);
    background: var(--easytv-card-background);
    border: var(--easytv-card-border);
    box-shadow: var(--easytv-card-box-shadow);
    backdrop-filter: var(--easytv-card-backdrop-filter);
    -webkit-backdrop-filter: var(--easytv-card-backdrop-filter);
  ...
  .compact-double .d-bottom .qa-btn {
    ...
    border: var(--easytv-button-border);
    color: var(--easytv-text-color);
  ...
  .icon-btn {
    ...
    border-radius: var(--easytv-button-border-radius);
    display: flex; align-items: center; justify-content: center;
...
const OVERLAY_STYLES = `
  ...
  #easytv-overlay .etv-section {
    ...
    border-radius: var(--easytv-overlay-section-radius);
  ...
  /* ── Legacy dpad-grid (kept for fallback safety, hidden when svg-dpad present) ── */
  ...
  #easytv-overlay .svg-dpad-wrap {
    ...
    width: 100%;
    padding-bottom: 100%;
    /* square aspect ratio via padding trick — no JS sizing needed, Pi safe */
  }
  ...
  /* Petal hit areas — transparent by default, fill on hover/active */
  ...
  #easytv-overlay .dpad-select-circle:active { fill: color-mix(in srgb, var(--primary-color, #1976d2) 45%, rgba(255,255,255,0.10)); transform: scale(0.91); }
  /* Directional icons rendered as foreignObject ha-icon inside SVG */
  #easytv-overlay .dpad-icon-fo {
    pointer-events: none;
    overflow: visible;
  }
...
const EDITOR_STYLES = `
  ...
  .qa-item {
    display: grid; align-items: center; grid-template-columns: 28px 1fr auto;
    gap: 8px; padding: 10px 12px; border-radius: 10px;
    background: var(--secondary-background-color, #2a2a2a);
    border: 1px solid var(--divider-color, rgba(255,255,255,0.15));
  }
  .editor-note { font-size: 12px; line-height: 1.45; color: var(--secondary-text-color, rgba(255,255,255,0.6)); padding: 2px 2px 0; }
`;
...
function normalizeSections(sections) {
  if (Array.isArray(sections)) {
    ...
    const result = sections
      ...
      .map(s => { seen.add(s.id); return { id: s.id, enabled: s.enabled !== false, width: normalizeWidth(s.width) }; });
    SECTION_ORDER.forEach(id => {
      if (!seen.has(id)) result.push({ id, enabled: id !== 'numpad', width: (id === 'power') ? 'quarter' : (id === 'app_selector') ? 'three-quarter' : 'full' });
    });
    return result;
  }
...
function autoSortSections(sections) {
  return [...sections.filter(s => s.enabled !== false), ...sections.filter(s => s.enabled === false)];
}
...
function editorInput(value, placeholder) {
  const el = document.createElement('input'); el.className = 'etv-input'; el.type = 'text';
  el.value = value || ''; el.placeholder = placeholder || ''; return el;
}
function editorTextarea(value, placeholder) {
  const el = document.createElement('textarea'); el.className = 'etv-textarea';
  el.value = value || ''; el.placeholder = placeholder || ''; el.spellcheck = false; return el;
}
function editorSelect(options, currentValue, className = 'etv-select') {
  ...
  options.forEach(([val, label]) => {
    ...
    if (val === currentValue) o.selected = true; el.appendChild(o);
  }); return el;
}
function editorRow(labelText, switchEl) {
  const row = document.createElement('div'); row.className = 'row';
  const lbl = document.createElement('label'); lbl.textContent = labelText;
  row.appendChild(lbl); row.appendChild(switchEl); return row;
}
function editorSwitch(checked) {
  const el = document.createElement('ha-switch'); if (checked) el.setAttribute('checked', ''); return el;
}
function editorH3(text) { const h = document.createElement('h3'); h.textContent = text; return h; }
function editorPanel(title, desc) {
  const panel = document.createElement('div'); panel.className = 'editor-panel';
  const header = document.createElement('div'); header.className = 'editor-panel-header';
  const titleEl = document.createElement('div'); titleEl.className = 'editor-panel-title'; titleEl.textContent = title;
  const descEl = document.createElement('div'); descEl.className = 'editor-panel-desc'; descEl.textContent = desc;
  header.appendChild(titleEl); header.appendChild(descEl); panel.appendChild(header); return panel;
}
...
class EasyTVCard extends HTMLElement {
  ...
  _applyOverlayTheme(overlay) {
    ...
    overlay.style.setProperty('--easytv-overlay-btn-background',   t.buttonBackground);
    overlay.style.setProperty('--easytv-overlay-text-color',       t.textColor);
    overlay.style.setProperty('--easytv-overlay-muted-color',      t.mutedColor);
    overlay.style.setProperty('--easytv-overlay-border-color',     t.borderColor);
    ...
    dynStyle.textContent = `
      ...
      #easytv-overlay .etv-section {
        background: var(--easytv-overlay-section-bg, ${showSectionBg ? t.sectionBackground : 'transparent'});
        border: ${showSectionBg ? `1px solid ${t.borderColor}` : '1px solid transparent'};
      }
      #easytv-overlay .section-label { color: var(--easytv-overlay-muted-color, ${t.mutedColor}); }
      #easytv-overlay .icon-btn {
        ...
        color: var(--easytv-overlay-text-color, ${t.textColor});
      }
  ...
  _compactSingle() {
    ...
    if (show_name !== false) {
      const s = document.createElement('span'); s.className = 'tv-name'; s.textContent = name || 'My TV';
      left.appendChild(s);
    }
  ...
  get _showLabels() { return this._config.overlay_show_labels !== false; }

  _buildPower() {
    const c = this._commands;
    const wrap = sectionWrap('Power', this._showLabels); wrap.style.flex = '1';
    const row = document.createElement('div'); row.className = 'btn-row power-only-row';
    row.appendChild(iconBtn('mdi:power', () => this._send(c.power || 'power'), 'Power'));
  ...
  _buildAppSelector() {
    ...
    (state.attributes.options || []).forEach(opt => {
      ...
      if (opt === state.state) o.selected = true; sel.appendChild(o);
    });
  ...
  /**
   * _buildDpad — v0.5.0
   * Renders a pure SVG arc d-pad (four petal shapes + centre select circle).
   * No canvas, no JS sizing loops. Just static SVG paths + CSS transitions.
   * Pi-safe: zero runtime cost after first paint.
   *
   * SVG coordinate space: 100×100 viewBox.
   * Centre: (50,50). Outer radius: 50. Inner radius (hole): 18. Select circle radius: 15.
   *
   * Each petal is an SVG <path> drawn as an annular sector:
   *   - Outer arc from angleStart to angleEnd at r=48 (slight inset for gap)
   *   - Inner arc from angleEnd to angleStart at r=20
   *   - Two straight lines connecting them
   * Gap between petals: 4° on each side of the 90° boundary.
   */
  _buildDpad() {
    const c = this._commands;
    ...
    // Helper: polar to cartesian (cx=50, cy=50)
    function pt(angleDeg, r) {
      const rad = (angleDeg - 90) * Math.PI / 180;
      return [50 + r * Math.cos(rad), 50 + r * Math.sin(rad)];
    }

    // Build annular sector path: outer arc a1→a2, inner arc a2→a1
    ...
    // 4 petals: Up, Right, Down, Left
    // Each covers 82° centred on its cardinal direction (4° gap each side)
    const petals = [
      { label: 'Up',    cmd: c.up,    icon: 'mdi:arrow-up-bold',    a1: -41+360, a2:  41,      iconAng: 0   },
    ...
    const OUTER_R = 48;
    const INNER_R = 20;
    const ICON_R  = 35; // radius at which icon centres sit
    ...
    petals.forEach(({ label, cmd, icon, a1, a2, iconAng }) => {
      ...
      // Icon via foreignObject — renders ha-icon at the petal centre
      const [iconX, iconY] = pt(iconAng, ICON_R);
      const SIZE = 16; // foreignObject box size (px in viewBox units)
    ...
    // Back + Home auxiliary row below the d-pad
  ...
  _buildAppShortcuts() {
    ...
    apps.forEach(app => {
      ...
      const lbl = document.createElement('span'); lbl.textContent = app.name;
      btn.appendChild(lbl);
  ...
  _render() {
    ...
    root.innerHTML = '';
    const style = document.createElement('style'); style.textContent = CARD_STYLES;
    root.appendChild(style);
    if (this._config.card_mod?.style) {
      const modStyle = document.createElement('style'); modStyle.textContent = this._config.card_mod.style;
...
class EasyTVCardEditor extends HTMLElement {
  ...
  _removeQA(index) {
    const qa = [...(this._config.quick_actions || this._defaultQA())];
    qa.splice(index, 1); this._setQuickActions(qa);
  }
  ...
  _renderSectionEditor(parent) {
    ...
    list.addEventListener('click', (e) => {
      const moveBtn = e.target.closest('[data-section-action]'); if (!moveBtn) return;
      const item = moveBtn.closest('.section-item'); if (!item) return;
    ...
    list.addEventListener('change', (e) => {
      const widthSel = e.target.closest('.section-width'); if (!widthSel) return;
      const item = widthSel.closest('.section-item'); if (!item) return;
      const idx = parseInt(item.dataset.sectionIndex, 10);
    ...
    this._config.sections.forEach((section, index) => {
      ...
      const row1 = document.createElement('div'); row1.className = 'section-row1';
      const handle = document.createElement('button'); handle.className = 'section-handle';
      handle.textContent = '\u283f'; handle.title = 'Drag handle (coming soon)';
      ...
      row1.appendChild(handle); row1.appendChild(nameEl); row1.appendChild(sw);
      item.appendChild(row1);
      const row2 = document.createElement('div');
      row2.className = 'section-row2' + (enabled ? ' open' : '');
      const widthSel = document.createElement('select'); widthSel.className = 'section-width';
      [['full','Full width'],['three-quarter','\u00be width'],['half','\u00bd width'],['quarter','\u00bc width']].forEach(([val, lbl]) => {
        ...
        if (val === normalizeWidth(section.width)) o.selected = true; widthSel.appendChild(o);
      });
      const upBtn = document.createElement('button'); upBtn.className = 'section-move';
      upBtn.textContent = '\u2191'; upBtn.title = 'Move up'; upBtn.dataset.sectionAction = 'up';
      const dnBtn = document.createElement('button'); dnBtn.className = 'section-move';
      dnBtn.textContent = '\u2193'; dnBtn.title = 'Move down'; dnBtn.dataset.sectionAction = 'down';
      row2.appendChild(widthSel); row2.appendChild(upBtn); row2.appendChild(dnBtn);
  ...
  _renderQAEditor(parent) {
    ...
    list.addEventListener('click', (e) => {
      const btn = e.target.closest('[data-qa-action]'); if (!btn) return;
      const item = btn.closest('.qa-item'); if (!item) return;
    ...
    qa.slice(0, limit).forEach((key, index) => {
      const def = QUICK_ACTION_DEFS[key]; if (!def) return;
      const item = document.createElement('div'); item.className = 'qa-item'; item.dataset.qaIndex = String(index);
      ...
      const up = document.createElement('button'); up.className = 'section-move'; up.textContent = '\u2191'; up.dataset.qaAction = 'up';
      const dn = document.createElement('button'); dn.className = 'section-move'; dn.textContent = '\u2193'; dn.dataset.qaAction = 'down';
      const rm = document.createElement('button'); rm.className = 'section-move'; rm.textContent = '\u2715'; rm.style.color = '#e74c3c'; rm.dataset.qaAction = 'remove';
  ...
  _render() {
    ...
    const root = this.shadowRoot;
    root.innerHTML = '';
    const style = document.createElement('style'); style.textContent = EDITOR_STYLES;
    root.appendChild(style);
    ...
    remotePicker.id = 'etv-remote'; if (this._hass) remotePicker.hass = this._hass;
    ...
    appPicker.id = 'etv-appselect'; if (this._hass) appPicker.hass = this._hass;
    ...
    const cssNote = document.createElement('div'); cssNote.className = 'editor-note';
    cssNote.textContent = 'Applied to both the compact card (shadow DOM) and the pop-out overlay. Use --easytv-* custom properties for theming — see docs for the full list.';
    globalPanel.appendChild(cssNote);
    const cssEl = editorTextarea(c.card_mod?.style || '', '/* Examples:\n--easytv-card-background: transparent;\n--easytv-card-backdrop-filter: blur(12px);\n--easytv-card-border-radius: 28px;\n*/');
