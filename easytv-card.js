...
class EasyTVCard extends HTMLElement {
  ...
  _buildCard() {
    ...
    const enabledSet  = new Set(cfg.compact_buttons !== undefined ? cfg.compact_buttons : DEFAULT_COMPACT_BUTTONS);
    const orderedKeys = (cfg.compact_button_order || DEFAULT_COMPACT_BUTTONS).filter(k => enabledSet.has(k));
    const makeBtn = (key, cls) => {
      const def = COMPACT_BUTTON_DEFS.find(b => b.key === key);
      if (!def) return '';
      return `<div class="${cls}" data-action="quick-action" data-qa="${key}" title="${def.label}"><ha-icon icon="${def.icon}"></ha-icon></div>`;
    };

    if (cardType === 'single') {
      const qaPillBtns = orderedKeys.map(k => makeBtn(k, 'qa-pill-btn')).join('');
      wrapper.innerHTML = `
      ...
      const qaRowBtns = orderedKeys.map(k => makeBtn(k, 'd-qa-btn')).join('');
      wrapper.innerHTML = `
...
class EasyTVCardEditor extends HTMLElement {
  ...
  _render() {
    ...
    compactPanel.innerHTML = `
      ...
      <div class="etv-section">
        <div class="section-title">Sub Buttons</div>
        <div style="font-size:11px;color:var(--secondary-text-color,rgba(255,255,255,0.5));margin-bottom:4px;">
          Toggle visibility and use &#x25B2;&#x25BC; to reorder buttons on the compact card.
        </div>
        <div id="btn-order-list" style="display:flex;flex-direction:column;gap:6px;"></div>
      </div>
    `;
    panels.appendChild(compactPanel);

    // ── Wire up sub-button order/visibility list
    {
      const enabledKeys = cfg.compact_buttons !== undefined
        ? [...cfg.compact_buttons]
        : [...DEFAULT_COMPACT_BUTTONS];
      const orderKeys = cfg.compact_button_order !== undefined
        ? [...cfg.compact_button_order]
        : [...DEFAULT_COMPACT_BUTTONS];
      const allInOrder = [
        ...orderKeys,
        ...DEFAULT_COMPACT_BUTTONS.filter(k => !orderKeys.includes(k)),
      ];
      const listEl = compactPanel.querySelector('#btn-order-list');
      const saveButtonConfig = () => {
        const rows = listEl.querySelectorAll('[data-btn-key]');
        const newOrder   = [...rows].map(r => r.dataset.btnKey);
        const newEnabled = [...rows].filter(r => r.querySelector('ha-switch').checked).map(r => r.dataset.btnKey);
        this._config = { ...this._config, compact_button_order: newOrder, compact_buttons: newEnabled };
        this._fireChange();
      };
      allInOrder.forEach(key => {
        const def = COMPACT_BUTTON_DEFS.find(b => b.key === key);
        if (!def) return;
        const row = document.createElement('div');
        row.dataset.btnKey = key;
        row.style.cssText = 'display:flex;align-items:center;gap:8px;padding:6px 8px;border-radius:8px;background:var(--secondary-background-color,rgba(255,255,255,0.04));border:1px solid var(--divider-color,rgba(255,255,255,0.08));';
        const iconEl = document.createElement('ha-icon');
        iconEl.setAttribute('icon', def.icon);
        iconEl.style.cssText = '--mdc-icon-size:18px;flex-shrink:0;color:var(--primary-text-color,#fff);';
        row.appendChild(iconEl);
        const labelEl = document.createElement('span');
        labelEl.textContent = def.label;
        labelEl.style.cssText = 'flex:1;font-size:13px;color:var(--primary-text-color,#fff);';
        row.appendChild(labelEl);
        const upBtn = document.createElement('button');
        upBtn.textContent = '\u25b2';
        upBtn.title = 'Move up';
        upBtn.style.cssText = 'background:none;border:none;color:var(--secondary-text-color,rgba(255,255,255,0.5));cursor:pointer;font-size:12px;padding:2px 5px;';
        upBtn.addEventListener('click', () => { const p = row.previousElementSibling; if (p) { listEl.insertBefore(row, p); saveButtonConfig(); } });
        row.appendChild(upBtn);
        const downBtn = document.createElement('button');
        downBtn.textContent = '\u25bc';
        downBtn.title = 'Move down';
        downBtn.style.cssText = 'background:none;border:none;color:var(--secondary-text-color,rgba(255,255,255,0.5));cursor:pointer;font-size:12px;padding:2px 5px;';
        downBtn.addEventListener('click', () => { const n = row.nextElementSibling; if (n) { listEl.insertBefore(n, row); saveButtonConfig(); } });
        row.appendChild(downBtn);
        const sw = document.createElement('ha-switch');
        sw.checked = enabledKeys.includes(key);
        sw.addEventListener('change', () => saveButtonConfig());
        row.appendChild(sw);
    ...
    // ── Wire up switches (global/overlay toggles only — compact buttons handled above)
